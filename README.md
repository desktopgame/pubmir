...
`exclude` はファイルの存在ごと mirror から消しますが、それでは AI が「そのファイルがある」ことすら分からなくなります。**stub** は、中身だけを隠してパスと存在は見せたい場合に使います。証明書や鍵ファイルのように、存在自体が構成の理解に役立つものを `exclude` の代わりに残す用途が典型的です。
...
秘密鍵・アクセストークン・パスワードなどの**値そのもの**は、そもそも Git 管理しないことを基本としています。既定の `exclude` も `**/*.key` や `**/*.pem` を mirror から除外します。

一方で、証明書や鍵ファイルのように**ファイルが存在すること自体が構成の理解に役立つ**ものは、`exclude` で存在ごと消すのではなく `stub` を使うことを推奨します。mirror 側にはパスと存在だけが残り、中身は placeholder に置き換わるため、秘密を渡さずに構造を保てます。既定の `exclude` に含まれるパターンを stub で扱いたい場合は、`exclude` からそのパターンを外して `stub` へ移してください。
