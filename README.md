...
秘密鍵・アクセストークン・パスワードなどを元の非公開リポジトリで管理する場合でも、mirror に実値を出さないよう exclude または stub の対象にしてください。

証明書や鍵ファイルのように、ファイルが存在すること自体が構成の理解に役立つものは、exclude で存在ごと消すのではなく stub を使うことを推奨します。

ただし、既定の exclude では鍵ファイルをミラーから除外します。パスと存在を残したい場合は、該当パターンを exclude から外し、stub に設定してください。
